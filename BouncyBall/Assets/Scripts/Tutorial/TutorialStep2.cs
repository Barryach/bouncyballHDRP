...
public class TutorialStep2 : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player") && !isTyping) // Evita que se active mientras se escribe
        {
            ...
            string message = "¡Cuidado con la lava! Si el charco no es muy grande, puedes saltarlo. Si no, necesitarás transformarte en la bola de roca para pasar sin problemas.";
    ...
    void Update()
    {
        // Verifica si el jugador ha cambiado a la bola de roca (suponiendo que el tipo 3 es la bola de roca)
